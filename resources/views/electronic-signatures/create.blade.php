 ...
 <form method="POST" enctype="multipart/form-data" action="{{route($pageMeta['store'] ?? 'electronic-signatures.store')}}" class="card border-0 shadow-sm">@csrf
  <div class="card-body p-4">
   ...
   <div id="jctSource" class="rounded bg-light p-3 mb-4">
    <div class="d-flex flex-wrap justify-content-between gap-2 mb-3"><div><strong>Sertifikat Jabar Corpu Talent</strong><small class="d-block text-muted">Nama kegiatan dibaca dari API JCT; peserta sertifikat dibaca melalui database bridge.</small></div><div class="d-flex gap-2"><span class="badge bg-label-{{$jctError?'danger':'success'}}">API {{$jctError?'bermasalah':'terhubung'}}</span><span class="badge bg-label-{{$jctDbConnected?'success':'warning'}}">Bridge {{$jctDbConnected?'terhubung':'belum terhubung'}}</span></div></div>
    @if($jctError)<div class="alert alert-danger mb-3"><i class="bx bx-error-circle me-1"></i>{{$jctError}}</div>@endif
    @if($jctWarning)<div class="alert alert-warning mb-3"><i class="bx bx-info-circle me-1"></i>{{$jctWarning}} Daftar kegiatan tetap dapat dilihat, tetapi jumlah sertifikat dan proses pengajuan belum dapat dijalankan.</div>@endif
    <div class="row g-3"><div class="col-12"><label class="form-label">Pilih kegiatan dari JCT *</label><select id="jctOption" class="form-select jct-required"><option value="">Pilih data pelatihan Jabar Corpu Talent</option>@foreach($jctOptions as $option)<option value="{{$option['template_id']}}" data-activity="{{$option['activity_id']}}" data-name="{{$option['name']}}" data-users='@json($option['user_ids'])'>{{$option['name']}} &middot; {{$jctDbConnected ? count($option['user_ids']).' sertifikat' : 'jumlah belum terbaca'}}</option>@endforeach</select><small class="text-muted d-block mt-1">Ketik untuk mencari kegiatan berdasarkan nama pelatihan.</small>
     <div class="d-flex flex-wrap align-items-center gap-2 mt-3"><button type="button" id="jctCheckDownload" class="btn btn-outline-info btn-sm" data-bridge-ready="{{$jctDbConnected?'1':'0'}}" disabled><i class="bx bx-download me-1"></i>Cek &amp; Download Sertifikat</button><span id="jctDownloadSummary" class="small text-muted">{{$jctDbConnected?'Pilih kegiatan terlebih dahulu.':'Jumlah sertifikat belum tersedia sampai bridge JCT terhubung.'}}</span></div>
    </div><input type="hidden" name="jct_training_name" id="jctTrainingName"><input type="hidden" name="jct_activity_id" id="jctActivityId"><input type="hidden" name="jct_template_id" id="jctTemplateId"><textarea class="d-none" name="jct_user_ids" id="jctUserIds"></textarea>@if(!$jctError && $jctOptions->isEmpty())<div class="col-12"><div class="alert alert-warning mb-0">JCT belum mengirim daftar kegiatan sertifikat.</div></div>@endif
    @if($jctDbConnected)<small class="text-muted d-block mt-2">Daftar peserta berhasil dibaca; status TTE akan disinkronkan kembali ke JCT.</small>@endif
    </div>
   </div>
   @endif
...
function applyJctSelection(){const el=document.getElementById('jctOption');const option=el?.selectedOptions[0];document.getElementById('jctActivityId').value=option?.dataset.activity||'';document.getElementById('jctTemplateId').value=option?.value||'';document.getElementById('jctTrainingName').value=option?.dataset.name||'';document.getElementById('jctUserIds').value=JSON.parse(option?.dataset.users||'[]').join('\n');forceJctDownloadButtonState();}
    ...
    function forceJctDownloadButtonState(){const el=document.getElementById('jctOption');const button=document.getElementById('jctCheckDownload');if(!button)return;const bridgeReady=button.dataset.bridgeReady==='1';const selected=el?.value&&el?.selectedOptions[0]?.dataset?.activity;button.disabled=!selected||!bridgeReady;const summary=document.getElementById('jctDownloadSummary');if(!bridgeReady){summary.textContent='Jumlah sertifikat belum tersedia sampai bridge JCT terhubung.';}else if(!selected){summary.textContent='Pilih kegiatan terlebih dahulu.';}else{summary.textContent=el.selectedOptions[0].dataset.name+' · cek status dan download sertifikat sebelum mengajukan.';}}
