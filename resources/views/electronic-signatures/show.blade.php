...
<div class="container-xxl container-p-y">
 ...
 <div class="card border-0 shadow-sm mb-4"><div class="card-body"><div class="d-flex justify-content-between mb-2"><strong>Progres Bundel</strong><strong>{{$completedDocuments}}/{{$totalDocuments}} Â· {{$percent}}%</strong></div><div class="progress" style="height:12px"><div class="progress-bar bg-success" style="width:{{$percent}}%"></div></div></div></div>
 <div class="card border-0 shadow-sm mb-4"><div class="card-header"><h6 class="fw-bold mb-1">Alur Persetujuan</h6><small class="text-muted">Tombol tanda tangan muncul otomatis ketika seluruh tahapan sebelumnya selesai.</small></div><div class="card-body"><div class="row g-3">@foreach($signatureRequest->actors as $actor)@php($done=$actor->actions->isNotEmpty()&&$actor->actions->every(fn($action)=>$action->status==='completed'))<div class="col-md-4"><div class="border rounded p-3 h-100 d-flex gap-3"><span class="avatar-initial rounded-circle bg-label-{{$done?'success':'secondary'}} p-2">{{$actor->sequence}}</span><div><strong>{{$actor->user?->name}}</strong><small class="d-block text-muted">{{$actor->role==='signer'?'Penandatangan akhir':'Pemaraf'}} Â· {{$actor->user?->nip_nik}}</small><span class="badge mt-2 bg-label-{{$done?'success':'secondary'}}">{{$done?'Selesai':'Menunggu'}}</span></div></div></div>@endforeach</div></div></div>
 <div class="card border-0 shadow-sm"><div class="card-header d-flex justify-content-between"><div><h6 class="fw-bold mb-1">Daftar Dokumen</h6><small class="text-muted">Status setiap berkas dalam bundel.</small></div></div><div class="table-responsive"><table class="table table-hover align-middle mb-0"><thead><tr><th>Dokumen/Peserta</th><th>Ukuran</th><th>Posisi Proses</th><th>Status</th><th></th></tr></thead><tbody>@foreach($signatureRequest->documents as $document)@php($current=$document->actions->firstWhere('status','pending'))<tr><td><strong>{{$document->participantCertificate?->participant?->name?:$document->original_name}}</strong>@if($document->participantCertificate)<small class="d-block text-muted">{{$document->participantCertificate->participant?->nip_nik}} Â· {{$document->participantCertificate->certificate_number}}</small>@endif</td><td>{{number_format($document->file_size/1024,1)}} KB</td><td><small>{{$current?'Menunggu '.$current->actor?->user?->name:'Seluruh tahapan selesai'}}</small></td><td><span class="badge bg-label-{{$document->status==='completed'?'success':'warning'}}">{{$document->status==='completed'?'Ditandatangani':'Diproses'}}</span></td><td class="text-end"><a href="{{route('electronic-signatures.download',$document)}}" class="btn btn-sm btn-outline-primary"><i class="bx bx-download"></i></a></td></tr>@endforeach</tbody></table></div></div>
</div>
...
batchStart.addEventListener('click', async function () {
    ...
    for (let i = 0; i < actionsBeingProcessed.length; i++) {
        ...
        const done = i + 1;
        batchCounter.textContent = `${done}/${actionsBeingProcessed.length} · ${success} berhasil · ${failed} gagal`;
