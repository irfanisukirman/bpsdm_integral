    ...
    $sections = [
        'preparation' => [
            ...
            'items' => [
                ...
                ['title' => 'Desain ID Card', 'description' => 'Logo, warna, latar, dan kartu peserta 9,2 × 12,4 cm', 'icon' => 'bx-id-card', 'tone' => 'primary', 'url' => route('training-id-cards.settings', $training), 'status' => $training->identityCardSetting?->enabled ? 'Aktif' : 'Atur desain', 'status_tone' => $training->identityCardSetting?->enabled ? 'success' : 'warning'],
            ],
        ],
        'execution' => [
...
<div class="training-command-center">
    ...
    <section class="registration-code-card mb-3" aria-label="Kode pendaftaran peserta">
        <div class="registration-code-card__icon"><i class="bx bx-key"></i></div>
        <div class="flex-grow-1 min-w-0">
            <small class="text-uppercase">Kode pendaftaran peserta</small>
            <div class="d-flex flex-wrap align-items-center gap-2"><strong id="trainingInvitationCode">{{ $training->invitation_code }}</strong><span>Bagikan kode ini kepada peserta untuk pendaftaran awal.</span></div>
        </div>
        <button type="button" class="btn btn-primary flex-shrink-0" id="copyTrainingCode" data-code="{{ $training->invitation_code }}"><i class="bx bx-copy me-1"></i><span>Salin Kode</span></button>
    </section>
...
document.getElementById('copyTrainingCode')?.addEventListener('click', async function () {
    ...
    try {
        ...
        window.prompt('Salin kode pendaftaran berikut:', code);
    }
});
</script>
@endpush
...
@media(max-width:767.98px){.registration-code-card{align-items:flex-start;flex-wrap:wrap}.registration-code-card .btn{width:100%}.training-hero{padding:1.25rem}.training-hero__icon{display:none}.hero-actions .btn{flex:1}.action-grid,.download-list{grid-template-columns:1fr}.attention-item{align-items:flex-start;padding:1rem}.attention-action{font-size:0}.attention-action i{font-size:1.3rem}.summary-card span{white-space:normal}.workflow-content{padding:1rem}.workflow-head{padding:1rem}.all-clear{align-items:flex-start;padding:1.5rem}}
