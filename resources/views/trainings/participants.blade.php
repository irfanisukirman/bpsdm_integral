...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        @if($search)
            <div class="mb-3 animate__animated animate__fadeIn">
                <small class="text-muted">Menampilkan hasil pencarian untuk: <span
                        class="fw-bold text-primary">"{{ $search }}"</span></small>
            </div>
        @endif
                        ...
                        <tbody class="table-border-bottom-0">
                            @forelse($participants as $p)
                                <tr class="participant-row">
                                    ...
                                    <td class="align-top">
                                        <div class="d-flex align-items-center">
                                            <code class="fw-bold text-danger me-2" id="nip-{{ $p->id }}"
                                                style="font-size: 0.8rem;">{{ $p->nip_nik }}</code>
                                            <button class="btn btn-xs btn-icon btn-outline-secondary border-0"
                                                onclick="copyToClipboard('{{ $p->nip_nik }}', this)" title="Salin NIP">
                                    ...
                                    <td class="align-top text-wrap">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <span class="fw-bold text-dark"
                                                style="font-size: 0.85rem;">{{ strtoupper($p->name) }}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="align-top">
                                        <small class="d-block mb-1 text-muted">
                                            <i
                                                class="bx {{ $p->gender == 'Laki-Laki' ? 'bx-male-sign text-info' : 'bx-female-sign text-danger' }} me-1"></i>{{ $p->gender }}
                                        </small>
                                        @php
                                            $statusColor = 'bg-label-secondary';
                                            if (str_contains(strtoupper($p->status_kepegawaian), 'PNS'))
                                                $statusColor = 'bg-label-success';
                                            if (str_contains(strtoupper($p->status_kepegawaian), 'PPPK'))
                                                $statusColor = 'bg-label-warning';
                                            if (str_contains(strtoupper($p->status_kepegawaian), 'ASN'))
                                                $statusColor = 'bg-label-primary';
                                        @endphp
                                    ...
                                    <td class="align-top">
                                        @if($p->phone)
                                            <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $p->phone) }}" target="_blank"
                                    ...
                                    <td class="align-top text-center">
                                        <div class="d-flex justify-content-center gap-1">
                                            <button class="btn btn-sm btn-icon btn-label-warning"
                                                onclick="editParticipant({{ json_encode($p) }})" data-bs-toggle="modal"
                                                data-bs-target="#modalEdit" title="Ubah Data">
            ...
            <form action="{{ route('participants.store', $training->id) }}" method="POST" class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control" placeholder="Nama tanpa gelar disarankan"
                            required>
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-bold">Jabatan</label>
                            <input type="text" name="jabatan" class="form-control" placeholder="Contoh: Analis SDM"
                                required>
                        </div>
                        <div class="col-md-6 mb-3">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select id="add_kota" name="kota" class="form-select border-primary" required disabled>
                                <option value="">Pilih Provinsi Dahulu</option>
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-bold">Kecamatan</label>
                            <select id="add_kecamatan" name="kecamatan" class="form-select border-primary" required
                                disabled>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select id="add_kelurahan" name="kelurahan" class="form-select border-primary" required
                                disabled>
    ...
    <div class="modal fade" id="modalImport" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form action="{{ route('participants.import', $training->id) }}" method="POST" enctype="multipart/form-data"
                class="modal-content">
                @csrf
                <div class="modal-header border-bottom">
                ...
                <div class="modal-body">
                    <div class="alert alert-info shadow-none">
                        ...
                        <ul class="small mb-0">
                            <li>Gunakan dropdown yang tersedia pada kolom <strong>Gender, Status, dan Provinsi</strong>.
                            </li>
            ...
            <form action="" method="POST" id="formEditParticipant" class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-bold">Kota</label>
                            <select id="edit_kota" name="kota" class="form-select border-primary" required>
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-bold">Kecamatan</label>
                            <select id="edit_kecamatan" name="kecamatan" class="form-select border-primary" required>
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-bold">Kelurahan / Desa</label>
                            <select id="edit_kelurahan" name="kelurahan" class="form-select border-primary" required>
                            </select>
                        </div>
                    </div>
                </div>
...
@push('js')
    <script>
        ...
        $(document).ready(function () {
            ...
            // --- 2. PROVINSI -> KOTA ---
            $('#add_provinsi, #edit_provinsi').change(function () {
                const provinceId = $(this).find(':selected').data('id');
                const targetKota = $(this).attr('id') === 'add_provinsi' ? $('#add_kota') : $('#edit_kota');
                const targetKec = $(this).attr('id') === 'add_provinsi' ? $('#add_kecamatan') : $('#edit_kecamatan');
                const targetKel = $(this).attr('id') === 'add_provinsi' ? $('#add_kelurahan') : $('#edit_kelurahan');

                targetKec.prop('disabled', true).html('<option value="">Pilih Kota Dahulu</option>');
                targetKel.prop('disabled', true).html('<option value="">Pilih Kecamatan Dahulu</option>');

                if (provinceId) {
                    targetKota.prop('disabled', false).html('<option value="">Memuat...</option>');
                    fetch(`${apiKota}${provinceId}.json`)
                        .then(response => response.json())
                        .then(regencies => {
                            let opts = '<option value="">-- Pilih Kota --</option>';
                            regencies.forEach(r => { opts += `<option data-id="${r.id}" value="${r.name}">${r.name}</option>`; });
                            targetKota.html(opts);
                        });
                } else {
                    targetKota.prop('disabled', true).html('<option value="">Pilih Provinsi Dahulu</option>');
                }
            });

            // --- 3. KOTA -> KECAMATAN ---
            $('#add_kota, #edit_kota').change(function () {
                const cityId = $(this).find(':selected').data('id');
                const targetKec = $(this).attr('id') === 'add_kota' ? $('#add_kecamatan') : $('#edit_kecamatan');
                const targetKel = $(this).attr('id') === 'add_kota' ? $('#add_kelurahan') : $('#edit_kelurahan');

                targetKel.prop('disabled', true).html('<option value="">Pilih Kecamatan Dahulu</option>');

                if (cityId) {
                    targetKec.prop('disabled', false).html('<option value="">Memuat...</option>');
                    fetch(`${apiKecamatan}${cityId}.json`)
                        .then(response => response.json())
                        .then(districts => {
                            let opts = '<option value="">-- Pilih Kecamatan --</option>';
                            districts.forEach(d => { opts += `<option data-id="${d.id}" value="${d.name}">${d.name}</option>`; });
                            targetKec.html(opts);
                        });
                } else {
                    targetKec.prop('disabled', true).html('<option value="">Pilih Kota Dahulu</option>');
            ...
            $('#add_kecamatan, #edit_kecamatan').change(function () {
                const districtId = $(this).find(':selected').data('id');
                const targetKel = $(this).attr('id') === 'add_kecamatan' ? $('#add_kelurahan') : $('#edit_kelurahan');

                if (districtId) {
                    targetKel.prop('disabled', false).html('<option value="">Memuat...</option>');
                    fetch(`${apiKelurahan}${districtId}.json`)
                        .then(response => response.json())
                        .then(villages => {
                            let opts = '<option value="">-- Pilih Kelurahan --</option>';
                            villages.forEach(v => { opts += `<option data-id="${v.id}" value="${v.name}">${v.name}</option>`; });
                            targetKel.html(opts);
                        });
                } else {
                    targetKel.prop('disabled', true).html('<option value="">Pilih Kecamatan Dahulu</option>');
            ...
            // --- 5. LOGIKA LOAD DATA KE MODAL EDIT ---
                        ...
                        if (prov) {
                            fetch(`${apiKota}${prov.id}.json`)
                                ...
                                .then(regencies => {
                                    ...
                                    regencies.forEach(r => {
                                        const isSelected = (r.name === data.kota);
                                        if (isSelected) savedCityId = r.id;
                                        optsKota += `<option data-id="${r.id}" value="${r.name}" ${isSelected ? 'selected' : ''}>${r.name}</option>`;
                                    });
                                    $('#edit_kota').html(optsKota);

                                    if (savedCityId) {
                                        fetch(`${apiKecamatan}${savedCityId}.json`)
                                            .then(res => res.json())
                                            .then(districts => {
                                                let optsKec = '';
                                                let savedDistrictId = null;

                                                districts.forEach(d => {
                                                    const isKecSelected = (d.name === data.kecamatan);
                                                    if (isKecSelected) savedDistrictId = d.id;
                                                    optsKec += `<option data-id="${d.id}" value="${d.name}" ${isKecSelected ? 'selected' : ''}>${d.name}</option>`;
                                                });
                                                $('#edit_kecamatan').html(optsKec);

                                                if (savedDistrictId) {
                                                    fetch(`${apiKelurahan}${savedDistrictId}.json`)
                                                        .then(res => res.json())
                                                        .then(villages => {
                                                            let optsKel = '';
                                                            villages.forEach(v => {
                                                                const isKelSelected = (v.name === data.kelurahan);
                                                                optsKel += `<option data-id="${v.id}" value="${v.name}" ${isKelSelected ? 'selected' : ''}>${v.name}</option>`;
                                                            });
                                                            $('#edit_kelurahan').html(optsKel);
                                                        });
                                                }
                                            });
                                    }
