...
>>>>>>> afe4abe4f0ecd901dac2b05ee19acf60f25067fe
            </div>
        </div>
        @if ($search)
            <div class="mb-3 animate__animated animate__fadeIn">
                        ...
                        <tbody class="table-border-bottom-0">
                            @forelse($participants as $p)
                                <tr class="participant-row">
                                    ...
                                    <!-- Kolom Instansi & Wilayah -->
                                    <td class="align-top text-wrap">
                                        <div class="fw-bold text-dark mb-1" style="font-size: 0.8rem;">
                                            {{ $p->instansi }}</div>
                                        <small class="text-muted" style="font-size: 0.7rem;">
            ...
            <form action="{{ route('participants.store', $training->id) }}" method="POST" class="modal-content">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <input type="text" name="nip_nik" class="form-control" placeholder="Masukkan NIP/NIK"
                            required>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control"
                            placeholder="Nama tanpa gelar disarankan" required>
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-3">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="instansi" class="form-control" placeholder="Contoh: BPSDM"
                                required>
                        </div>
                    </div>
                    <div class="row">
    ...
    <div class="modal fade" id="modalImport" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form action="{{ route('participants.import', $training->id) }}" method="POST"
            ...
            <form action="" method="POST" id="formEditParticipant" class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-bold">Status Kepegawaian</label>
                            <input type="text" name="status_kepegawaian" id="edit_status" class="form-control"
                                required>
                        </div>
                    </div>
                    <div class="row">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#add_provinsi, #edit_provinsi').change(function() {
                ...
                const targetKec = $(this).attr('id') === 'add_provinsi' ? $('#add_kecamatan') : $(
                    '#edit_kecamatan');
                const targetKel = $(this).attr('id') === 'add_provinsi' ? $('#add_kelurahan') : $(
                    '#edit_kelurahan');
                ...
                if (provinceId) {
                    ...
                    targetKota.prop('disabled', true).html(
                        '<option value="">Pilih Provinsi Dahulu</option>');
                }
            });
            ...
            $('#add_kota, #edit_kota').change(function() {
                ...
                const targetKec = $(this).attr('id') === 'add_kota' ? $('#add_kecamatan') : $(
                    '#edit_kecamatan');
                const targetKel = $(this).attr('id') === 'add_kota' ? $('#add_kelurahan') : $(
                    '#edit_kelurahan');
            ...
            $('#add_kecamatan, #edit_kecamatan').change(function() {
                ...
                const targetKel = $(this).attr('id') === 'add_kecamatan' ? $('#add_kelurahan') : $(
                    '#edit_kelurahan');
                ...
                if (districtId) {
                    ...
                    targetKel.prop('disabled', true).html(
                        '<option value="">Pilih Kecamatan Dahulu</option>');
                }
            });
                            ...
                            fetch(`${apiKota}${prov.id}.json`)
                                ...
                                .then(regencies => {
                                    ...
                                    regencies.forEach(r => {
                                        ...
                                        optsKota +=
                                            `<option data-id="${r.id}" value="${r.name}" ${isSelected ? 'selected' : ''}>${r.name}</option>`;
                                    });
                                    ...
                                    if (savedCityId) {
                                        fetch(`${apiKecamatan}${savedCityId}.json`)
                                            ...
                                            .then(districts => {
                                                ...
                                                districts.forEach(d => {
                                                    const isKecSelected = (d.name === data
                                                        .kecamatan);
                                                    if (isKecSelected) savedDistrictId = d
                                                        .id;
                                                    optsKec +=
                                                        `<option data-id="${d.id}" value="${d.name}" ${isKecSelected ? 'selected' : ''}>${d.name}</option>`;
                                                });
                                                ...
                                                if (savedDistrictId) {
                                                    fetch(`${apiKelurahan}${savedDistrictId}.json`)
                                                        ...
                                                        .then(villages => {
                                                            let optsKel = '';
                                                            villages.forEach(v => {
                                                                const isKelSelected = (v
                                                                    .name === data
                                                                    .kelurahan);
                                                                optsKel +=
                                                                    `<option data-id="${v.id}" value="${v.name}" ${isKelSelected ? 'selected' : ''}>${v.name}</option>`;
                                                            });
