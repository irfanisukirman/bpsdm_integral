...
<div class="activity-report-page">
            ...
            <div class="card border-0 shadow-sm h-100">
                <div class="card-body p-4">
                    ...
                    <div class="metric-grid">
                        ...
                        <div class="metric-card metric-warning"><i class="bx bx-images"></i><div><strong>{{ $selectedPhotos }}</strong><span>Foto terpilih</span></div></div><div class="metric-card metric-purple"><i class="bx bx-transfer-alt"></i><div><strong>{{ $data['values']['nilai_evaluasi_l3'] }}</strong><span>Level 3</span></div></div><div class="metric-card metric-teal"><i class="bx bx-line-chart"></i><div><strong>{{ $data['values']['nilai_evaluasi_l4'] }}</strong><span>Level 4</span></div></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="report-tabs-shell mb-4">
        <div class="report-tabs-scroll">
            <ul class="nav nav-pills report-tabs flex-nowrap" role="tablist">
                ...
                <li class="nav-item"><button class="nav-link" data-bs-toggle="tab" data-bs-target="#evaluationSummary" type="button"><i class="bx bx-bar-chart-alt-2"></i><span>Evaluasi L3/L4</span><span class="tab-count">{{ $data['evaluation_l34_sections']->count() }}</span></button></li>
    ...
    <div class="tab-content report-tab-content p-0 bg-transparent shadow-none">
        ...
        <div class="tab-pane fade" id="evaluationSummary">
            <div class="card border-0 shadow-sm">
                <div class="card-header bg-transparent border-bottom p-4">
                    <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3"><div><h5 class="fw-bold mb-1">Ringkasan Evaluasi Level 3 dan 4</h5><p class="text-muted small mb-0">Bagian otomatis mengikuti Program Evaluasi {{ $training->program_evaluasi ?: 'PKTI/PKTU' }}.</p></div><a href="{{ route('evall34.dashboard', $training) }}" class="btn btn-outline-primary"><i class="bx bx-line-chart me-1"></i>Buka Dashboard Lengkap</a></div>
                </div>
                <div class="card-body p-4">
                    <div class="evaluation-headline mb-4"><div><small>Level 3 · {{ $data['values']['bagian_evaluasi_l3'] }}</small><strong>{{ $data['values']['nilai_evaluasi_l3'] }}</strong></div><div><small>Level 4 · {{ $data['values']['bagian_evaluasi_l4'] }}</small><strong>{{ $data['values']['nilai_evaluasi_l4'] }}</strong></div></div>
                    @if($data['evaluation_l34_sections']->isNotEmpty())
                        <div class="evaluation-section-grid">
                            @foreach($data['evaluation_l34_sections'] as $section)
                                @php $tone = $section['average'] === null ? 'secondary' : ($section['average'] >= 80 ? 'success' : ($section['average'] >= 70 ? 'warning' : 'danger')); @endphp
                                <article class="evaluation-section-item"><span class="evaluation-section-icon bg-label-{{ $tone }}"><i class="bx {{ str_contains($section['name'], 'Pendukung') ? 'bx-like' : (str_contains($section['name'], 'Penghambat') ? 'bx-error-circle' : 'bx-pulse') }}"></i></span><div class="flex-grow-1 min-w-0"><h6 class="fw-bold mb-1 text-break">{{ $section['name'] }}</h6><small class="text-muted">{{ $section['respondents'] }} alumni &middot; {{ $section['responses'] }} jawaban</small></div><div class="text-end">@if($section['average'] !== null)<strong class="text-{{ $tone }}">{{ number_format($section['average'],1,',','.') }}</strong><small class="d-block text-muted">/100</small>@else<span class="badge bg-label-secondary">Kualitatif</span>@endif</div></article>
                            @endforeach
                        </div>
                        <div class="report-code-hint mt-4"><i class="bx bx-code-alt"></i><div><strong>Kode ringkasan untuk template Word</strong><p class="mb-1">Gunakan <code>${ringkasan_evaluasi_l34}</code> untuk memasukkan rangkuman seluruh bagian dalam satu narasi.</p><small class="text-muted">Kode nilai dan jumlah respons masing-masing bagian tersedia pada tab Template &amp; Kode.</small></div></div>
                    @else
                        <div class="empty-state py-5"><span><i class="bx bx-bar-chart-square"></i></span><h5>Data evaluasi belum tersedia</h5><p>Bagian akan tampil setelah bank pertanyaan program tersedia.</p></div>
                    @endif
                </div>
            </div>
        </div>
