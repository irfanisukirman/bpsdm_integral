...
@php $editing=isset($agenda);$currentSchedule=$editing?$agenda->schedules->first():null;$currentAssetId=$currentSchedule?->bookings->first()?->asset_id; @endphp
...
<form method="POST" action="{{$editing?route('agendas.update',$agenda):route('agendas.store')}}">@csrf @if($editing) @method('PUT') @endif
