...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="animate_animated animate_fadeInRight">
            <button class="btn btn-primary btn-lg shadow-md px-4 pulse-button" data-bs-toggle="modal"
                data-bs-target="#modalJoinGlobal">
                    ...
                    hover-shadow-lg">
                        ...
                        <div class="card-header pb-2 d-flex justify-content-between align-items-center">
                            <div>
                                @if ($isPending)
                                    <span
                                        class="badge bg-label-warning animate__animated animate__flash animate__infinite">MENUNGGU
                                        APPROVAL</span>
                                @elseif($isRejected)
                                    <span class="badge bg-label-danger">DITOLAK</span>
                                @elseif($isExpired)
                                    <span class="badge bg-label-danger"><i class="bx bx-error-circle me-1"></i>PELATIHAN
                                        SELESAI</span>
                                @else
                                    <span class="badge bg-label-success"><i class="bx bx-play-circle me-1"></i>SEDANG
                                        BERJALAN</span>
                                @endif
                            </div>
                        ...
                        <div class="card-body">
                            <h5 class="card-title fw-extrabold text-dark mb-3" style="min-height: 50px;">
                                {{ $t->nama_pelatihan }}</h5>

                            {{-- Logika Tampilan Jadwal --}}
                            <div
                                class="mb-4 p-3 rounded-3 {{ $isExpired ? 'bg-label-danger' : 'bg-label-primary' }} text-center">
                                @if ($isExpired)
                                ...
                                @else
                                    <small class="d-block text-uppercase fw-bold mb-1" style="font-size: 10px;">Jadwal
                                        Pelaksanaan</small>
                            ...
                            @if ($isApproved)
                                <div class="task-section p-3 border rounded-3 bg-white">
                                    <h6 class="small fw-bold text-muted mb-3 text-uppercase border-bottom pb-2">Checklist
                                        Kewajiban:</h6>

                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <small>1. Kelengkapan Berkas</small>
                                        <i
                                            class="bx {{ $hasDocs ? 'bxs-check-circle text-success' : 'bx-x-circle text-danger animate__animated animate__heartBeat animate__infinite' }} fs-5"></i>
                                    </div>
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <small>2. Evaluasi Level 1</small>
                                        <i
                                            class="bx {{ $hasL1 ? 'bxs-check-circle text-success' : 'bx-x-circle text-danger animate__animated animate__heartBeat animate__infinite' }} fs-5"></i>
                                    </div>
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <small>3. Evaluasi Level 2</small>
                                        <i
                                            class="bx {{ $hasL2 ? 'bxs-check-circle text-success' : 'bx-x-circle text-danger' }} fs-5"></i>
                                    </div>
                                    <div class="d-flex justify-content-between align-items-center">
                                        <small>4. Evaluasi Dampak (L3&4)</small>
                                        <i
                                            class="bx {{ $hasL34 ? 'bxs-check-circle text-success' : 'bx-x-circle text-danger animate__animated animate__heartBeat animate__infinite' }} fs-5"></i>
                                    </div>
                                </div>
                            @elseif($isPending)
                                <div class="alert bg-label-secondary border-0 p-3 text-center">
                                    <i class="bx bx-loader-circle bx-spin me-1"></i> <small>Menunggu verifikasi NIP oleh
                                        Admin...</small>
                                </div>
                            @else
            ...
            <form action="{{ route('participant.training.join_by_code') }}" method="POST"
                ...
                <div class="modal-body text-center py-4">
                    ...
                    <input type="text" name="invitation_code"
                        ...
                        maxlength="6" style="letter-spacing: 5px; text-transform: uppercase;" required>
                </div>
