...
class AlumniExport implements FromArray, WithStyles, WithColumnWidths, WithEvents
{
    ...
    public function array(): array
    {
        ...
        $rows = [
            ...
            ['Wilayah 3T', $alumni->filter(fn($a) => in_array($a->kota, $list3T))->count(), $this->perc($alumni->filter(fn($a) => in_array($a->kota, $list3T))->count(), $total)],
            ['Non-3T', $alumni->filter(fn($a) => !in_array($a->kota, $list3T))->count(), $this->perc($alumni->filter(fn($a) => !in_array($a->kota, $list3T))->count(), $total)],
            [''],
